...
public class CodeExt extends Code {
	...
	public static class validateMessageExt extends validateMessage {
		...
		public static final String 아이디적합성오류 = "아이디는 4 ~ 12자리 영문 대소문자로 입력하세요.";
	}
	...
	public static class passwd {
		 ...
		 */
		public static void checkValidation(String passwd, String birthday, List<CmmtPasswdHist> passwdHistList) {
			if (!CoreUtils.password.isValid(passwd, 6)) {
				throw new InvalidationException("비밀번호는 숫자, 영문, 특수문자를 포함하여 6자리 이상이어야 합니다.");
			}
			...
			for(CmmtPasswdHist hist : passwdHistList) {
	...
	public static class loginId {
		...
		public static boolean isValid(String loginId)
		{
			...
			// 로그인ID 영문 대소문자를 제외한 문자가 포함되어 있는지 검증
        	if ( loginId.matches(".*[^a-zA-Z]+.*") ) {
        		return false;
        	}
        	return true;
		}
