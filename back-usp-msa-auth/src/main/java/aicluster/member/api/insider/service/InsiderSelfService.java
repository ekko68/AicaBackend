...
public class InsiderSelfService {
	...
	public void changePasswd(ChangePasswdParam param) {
		...
		CmmtInsider cmmtInsider = cmmtInsiderDao.select(worker.getMemberId());
		if (cmmtInsider == null) {
		...
		// 새 비밀번호 적합성 검사
		List<CmmtPasswdHist> passwdHistList = cmmtPasswdHistDao.selectList_recent(cmmtInsider.getMemberId(), 3);
