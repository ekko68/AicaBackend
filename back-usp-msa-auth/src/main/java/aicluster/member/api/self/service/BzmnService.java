...
public class BzmnService {
	...
	private SessionIdSetForBzmnDto getBzmnDto(String sessionId) 
	{
	...
	{
		checkWorkerIsChgBizTrgt();
		
		// 사업자전환 세션ID 모음 조회
		SessionIdSetForBzmnDto bzmnDto = getBzmnDto(sessionId);
		...
		if (string.isBlank(smsCn)) {
			throw new LoggableException("SMS인증번호 본문내용을 읽을 수 없습니다.");
		}

		String certNo = string.getRandomNo(6);
		SmsCertParam param = new SmsCertParam();
		param.setSmsCn(smsCn);
		param.addRecipient(mobileNo, "certNo", certNo);
		SmsResult sr = smsUtils.sendCert(param);
		if (sr.getFailCnt() == 1) {
			throw new InvalidationException("SMS 발송실패");
		}
	...
	public SessionKeyDto emailCertReq(String sessionId, String email)
	{
		 ...
		 */
		String certNo = string.getRandomNo(6);
		String emailCn = CoreUtils.file.readFileString("/form/email/email-cert.html");
		if (string.isBlank(emailCn)) {
			throw new LoggableException("이메일 인증용 본문 파일을 읽을 수 없습니다.");
		}

		EmailSendParam param = new EmailSendParam();
		param.setContentHtml(true);
		param.setEmailCn(emailCn);
		param.setTitle("(AICA) 이메일 인증번호");

		Map<String, String> templateParam = new HashMap<>();
		templateParam.put("certNo", certNo);
		param.addRecipient(email, worker.getMemberNm(), templateParam);
		 ...
		 * 인증번호 저장
		 */
		String emailSessionId = sessionUtils.emailCert.set(emailCn, certNo);
	...
	public void change(ChangeParam param)
	{
		...
		// 사업자전환 세션ID 모음 조회
		SessionIdSetForBzmnDto bzmnDto = getBzmnDto(param.getSessionId());
	...
	public SessionKeyDto bizrnoCert(PkiCertParam param)
	{
        ...
        CmmtMember member = cmmtMemberDao.selectByCi(pkiResult.getIdentifyData());
		if (member != null) {
			// 약관동의 임시저장 세션ID 만료 처리
			fwSessionUtils.expire(param.getTermsConsentSessionId());
			// 공동인증서 결과 세션ID 만료 처리
		...
		String termSessionId = fwSessionUtils.termsConsentSession.set(consentList);
