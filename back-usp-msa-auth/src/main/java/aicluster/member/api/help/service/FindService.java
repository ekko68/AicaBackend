...
public class FindService {
	...
	public LoginIdDto findIndividualLoginId(FindIndividualIdParam param) {
		...
		// 이름으로 조회
		List<CmmtMember> memberList = cmmtMemberDao.selectByMemberNm(param.getMemberNm());

		String birthday = null;
		String mobileNo = null;
		String email = null;
		for( CmmtMember cmmtMember : memberList ) {
			// 개인회원이 아니면 skip
	...
	public PasswdDto individualPasswd(IndividualPasswdParam param) {
		...
		CmmtMember cmmtMember = cmmtMemberDao.selectByLoginId(loginId);
		if (cmmtMember == null) {
			throw ie;
		}
	...
	public void phoneCertReq(PhoneCertReqParam param) {
		...
		if (sr.getSuccessCnt() == 0) {
			throw new InvalidationException("SMS 발송에 실패하였습니다.");
		}
	}
	...
	public void emailCertReq(EmailCertReqParam param) {
		...
		esp.setTitle("AICA) 인증번호");

		Map<String, String> templateParam = new HashMap<>();
		templateParam.put("certNo", certNo);
		esp.addRecipient(email, email, templateParam);
	...
	public void changePasswd(ChangePasswdParam param) {
		...
		// 비밀번호 규칙 확인
		List<CmmtPasswdHist> passwdHistList = cmmtPasswdHistDao.selectList_recent(memberId, 3);
