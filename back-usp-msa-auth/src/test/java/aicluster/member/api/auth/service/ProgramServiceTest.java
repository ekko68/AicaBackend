...
//	}
	...
	public void test() {
		...
		JsonList<CmmtProgram> programList = service.getList("portal", null, null);
		for (CmmtProgram program : programList.getList()) {
			System.out.println("\t\t" + program.toString());
		}
		System.out.println("\n\n\n");
		...
		List<CmmtProgram> insProgramRst = new ArrayList<>();
		for (ProgramDto program : initProgramList) {
			CmmtProgram insProgram = service.addProgram(program);
			insProgramRst.add(insProgram);
		}

		for (CmmtProgram program : insProgramRst) {
		...
		// 프로그램-역할 정보 조회
		CmmtProgram selProgram = service.getProgram(insProgramRst.get(0).getProgramId());
		...
		CmmtProgram upProgram = service.modifyProgram(updateProgram);
