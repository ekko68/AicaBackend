...
public class AuthorityServiceTest extends TestServiceSupport{
	...
	public void test() {
		...
		JsonList<CmmtAuthority> cmmtAuthorityList = service.getList();
		for (CmmtAuthority dto : cmmtAuthorityList.getList()) {
		...
		CmmtAuthority addAuthority = service.add(dto);
		System.out.println("권한-역할 등록 결과" + addAuthority.toString());
		System.out.println("\n\n\n");

		System.out.println("권한-역할 조회");
		CmmtAuthority getAuthority = service.get("USER");
		...
		CmmtAuthority  modifyAuthority = service.modify(dto);
