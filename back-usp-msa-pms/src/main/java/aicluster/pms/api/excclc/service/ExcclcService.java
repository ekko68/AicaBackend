...
public class ExcclcService {
	...
	public void modify(String bsnsCnvnId, ExcclcParam param, List<MultipartFile> fileList) {
		...
		List<CmmtAttachment> attachList = attachmentService.getFileInfos_group(info.getAttachmentGroupId());
		if(param.getDeleteFileList() != null && !param.getDeleteFileList().isEmpty()) {
			...
			String attachStr = attachList.stream().map(CmmtAttachment::getAttachmentId).collect(Collectors.joining());
			for(CmmtAttachment attachmenInfo : param.getDeleteFileList()) {
