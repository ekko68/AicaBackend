...
import aicluster.pms.api.selection.dto.LastSelectionDto;
...
public class SelectionService {
	 ...
	 * @return
	 */
	public JsonList<LastSelectionDto> modifyLastSelection(String evlLastSlctnId, List<LastSelectionParam> paramList) {
		BnMember worker = SecurityUtils.checkWorkerIsInsider();
		SlctnDetailDto slctnInfo = usptEvlLastSlctnDao.selectSlctnDetail(evlLastSlctnId, worker.getMemberId());
		if(slctnInfo == null) {
			throw new InvalidationException("요청하신 선정정보가 없습니다.");
		...
		List<LastSelectionDto> resultList = new ArrayList<LastSelectionDto>();
		Date now = new Date();
		for(LastSelectionParam param : paramList) {
			...
			if(slctnInfo.getWctMdatDlbrt() != null && slctnInfo.getWctMdatDlbrt()) {
			...
			if(CoreUtils.string.equals("Y", param.getSlctnAt())) {
			...
			if(CoreUtils.string.equalsIgnoreCase("Y", param.getSlctnAt())) {
				...
				LastSelectionDto dto = new LastSelectionDto();
				dto.setApplyId(trgetInfo.getApplyId());
				dto.setBsnsSlctnId(bsInfo.getBsnsSlctnId());
				resultList.add(dto);
			}
		}
		...
		return new JsonList<>(resultList);
	}
