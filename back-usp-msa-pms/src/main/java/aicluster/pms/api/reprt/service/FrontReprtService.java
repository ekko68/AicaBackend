...
public class FrontReprtService {
	...
	public void getMakeupAtchmnfl(HttpServletResponse response, String reprtId, String attachmentId) {
		...
		List<CmmtAttachment> fileList = attachmentService.getFileInfos_group(reprtInfo.getMakeupAttachmentGroupId());
	...
	public void getAtchmnfl(HttpServletResponse response, String reprtId, String attachmentId) {
		...
		List<CmmtAttachment> fileList = attachmentService.getFileInfos_group(reprtInfo.getAttachmentGroupId());
		Optional<CmmtAttachment> opt = fileList.stream().filter(x-> CoreUtils.string.equals(x.getAttachmentId(), attachmentId)).findFirst();
		if(opt.isPresent()) {
			attachmentService.downloadFile(response, config.getDir().getUpload(), attachmentId);
		} else {
