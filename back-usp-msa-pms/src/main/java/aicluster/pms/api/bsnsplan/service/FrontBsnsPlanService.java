...
public class FrontBsnsPlanService {
	...
	public CorePagination<UsptBsnsPlanDoc> getList(FrontBsnsPlanParam frontBsnsPlanParam,  Long page){
		...
		//전체건수 조회
		long totalItems = usptBsnsPlanDocDao.selectBsnsPlanDocCnt(frontBsnsPlanParam);
		...
		List<UsptBsnsPlanDoc> list = usptBsnsPlanDocDao.selectBsnsPlanDocList(frontBsnsPlanParam);

		if(list.size() == 0) {
			throw new InvalidationException("조회된 사업계획서 목록이 없습니다.");
		}
	...
	public FrontBsnsPlanResnDto getResnInfo(FrontBsnsPlanParam frontBsnsPlanParam){
		...
		if (string.isNotBlank(result.getMakeupReqFileGroupId())) {
			List<CmmtAttachment> list = attachmentService.getFileInfos_group(result.getMakeupReqFileGroupId());
	 ...
	 * @return
	 */
	public void getAllFileDown(HttpServletResponse response, FrontBsnsPlanParam frontBsnsPlanParam) {
		SecurityUtils.checkWorkerIsMember();
		if (string.isNotBlank(frontBsnsPlanParam.getMakeupReqFileGroupId())) {
			attachmentService.downloadGroupFiles(response, config.getDir().getUpload(), frontBsnsPlanParam.getMakeupReqFileGroupId(), "사업계획서 보안요청_첨부파일");
		}
	}
	...
	public FrontBsnsPlanDto getBsnsPlanDocInfo(FrontBsnsPlanParam frontBsnsPlanParam){
		...
		if(usptTaskRspnber != null) {
			...
			usptTaskRspnberInfo.setEncBrthdy(usptBsnsPblancApplyTask.getBrthdy()); 			/** 생년월일 */
			usptTaskRspnberInfo.setEncMbtlnum(usptBsnsPblancApplyTask.getMbtlnum());		/** 휴대폰번호 */
			usptTaskRspnberInfo.setEncEmail(usptBsnsPblancApplyTask.getEmail()); 				/** 이메일 */
			...
			usptTaskRspnberInfo.setEncTelno(usptBsnsPblancApplyTask.getTelno());				/** 전화번호 */
			usptTaskRspnberInfo.setEncFxnum(usptBsnsPblancApplyTask.getFxnum());  			/** 팩스번호 */
		...
		if(usptTaskPartcptsList.size()>0) {
			...
			for( UsptBsnsPblancApplyTaskPartcpts out :  usptBsnsPblancApplyTaskPartcptsList) {
				...
				usptTaskPartcptsReturn.setEncMbtlnum(out.getMbtlnum());	  			/** 휴대폰번호 */
				usptTaskPartcptsReturn.setEncBrthdy(out.getBrthdy());					/** 생년월일 */
		...
		if (string.isNotBlank(usptBsnsPlanDoc.getPrsentrAttachmentGroupId())) {
			List<CmmtAttachment> list = attachmentService.getFileInfos_group(usptBsnsPlanDoc.getPrsentrAttachmentGroupId());
	 ...
	 * @return
	 */
	public List<UsptTaskTaxitmWct> addTaxitmWct( TaskTaxitmWctDto taskTaxitmWctDto){
		/** 현재 시간*/
		Date now = new Date();
		/** 로그인 회원ID **/
	...
	public String insert( String planPresentnSttusCd, FrontBsnsPlanDto frontBsnsPlanDto, List<MultipartFile> fileList) {
		...
		/* 첨부파일 삭제 */
		List<CmmtAttachment> attachFileDeleteList = frontBsnsPlanDto.getAttachFileDeleteList();
		if( attachFileDeleteList !=null && attachFileDeleteList.size() >0) {
			for(CmmtAttachment attachInfo : attachFileDeleteList) {
				...
				if(!groupFileExists) {
					usptBsnsPlanDoc.setPrsentrAttachmentGroupId(null);
				}
			}
		}
		...
		usptBsnsPlanApplcnt.setChargerEmail(null);
		usptBsnsPlanApplcnt.setChargerMbtlnum(null);
	...
	public String modify( String planPresentnSttusCd, FrontBsnsPlanDto frontBsnsPlanDto, List<MultipartFile> fileList) {
		...
		if(usptTaskPrtcmpnyList.size() >0) {
			for(UsptTaskPrtcmpny input : usptTaskPrtcmpnyList) {
				String taskPartcptnEntrprsId = input.getTaskPartcptnEntrprsId();
				if(CoreUtils.string.isNotBlank(input.getRegTelno())) {
					input.setEncTelno(CoreUtils.aes256.encrypt(input.getRegTelno(), taskPartcptnEntrprsId));
				}
				if(CoreUtils.string.isNotBlank(input.getRegMbtlnum())) {
					input.setEncMbtlnum(CoreUtils.aes256.encrypt(input.getRegMbtlnum(), taskPartcptnEntrprsId));
				}
				if(CoreUtils.string.isNotBlank(input.getRegEmail())) {
					input.setEncEmail(CoreUtils.aes256.encrypt(input.getRegEmail(), taskPartcptnEntrprsId));
				}
				input.setUpdatedDt(now);
				input.setUpdaterId(strMemberId);
				input.setCreatedDt(now);
		...
		if(usptTaskPartcptsList.size() >0) {
			for(UsptTaskPartcpts input : usptTaskPartcptsList) {
				String taskPartcptsId = input.getTaskPartcptsId();

				if(CoreUtils.string.isNotBlank(input.getEncMbtlnum())) {
					input.setEncMbtlnum(CoreUtils.aes256.encrypt(input.getEncMbtlnum(), taskPartcptsId));
				}
				if(CoreUtils.string.isNotBlank(input.getEncBrthdy())) {
					input.setEncBrthdy(CoreUtils.aes256.encrypt(input.getEncBrthdy(), taskPartcptsId));
				}
				input.setCreatedDt(now);
				input.setCreatorId(strMemberId);
				input.setUpdatedDt(now);
				...
				if(CoreUtils.string.equals(Code.flag.등록, input.getFlag())){
					input.setTaskPartcptsId(CoreUtils.string.getNewId(Code.prefix.과제참여자));
					usptTaskPartcptsDao.insert(input);
				}else if(CoreUtils.string.equals(Code.flag.수정, input.getFlag())){
					usptTaskPartcptsDao.update(input);
				}else if(CoreUtils.string.equals(Code.flag.삭제, input.getFlag())){
					usptTaskPartcptsDao.delete(input);
				}
			}
		}
		/**과제신청사업비(신청예산)*/
	...
	public CorePagination<CmmtMemberBizDto> selectBizList(FrontBsnsPlanParam frontBsnsPlanParam,  Long page){
		...
		if(list.size() == 0) {
			throw new InvalidationException("조회된 기업 목록이 없습니다.");
		}
