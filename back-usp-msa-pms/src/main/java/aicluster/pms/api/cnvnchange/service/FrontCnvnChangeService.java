...
public class FrontCnvnChangeService {
	...
	public FrontCnvnChangeRequestDto selectCnvnChangeReqOne(FrontCnvnChangeParam frontCnvnChangeParam){
		...
		if(usptBsnsCnvnResult == null) {
			throw new InvalidationException("조회된 협약변경관리 신청내역이 없습니다.");
		}
	...
	public UsptCnvnSclpstHistDto selectChangeCnvnSclpstInfoFront(FrontCnvnChangeParam frontCnvnChangeParam){
		...
		//회원테이블에 담당자 정보 없음(협약 변경관리에서 등록 가능)
		usptCnvnSclpstHistAfter.setChargerMbtlnum(null);			/*복호화 담당자휴대폰번호*/
		usptCnvnSclpstHistAfter.setChargerEmail(null);		 		/*복호화 담당자이메일*/
	...
	public void modifyChangeCnvnSclpstFront(UsptCnvnSclpstHistDto usptCnvnSclpstHistDto,	List<MultipartFile> fileList) {
		...
		if(CoreUtils.string.isNotBlank(usptCnvnSclpstHist.getChargerMbtlnum())) {
			usptCnvnSclpstHist.setChargerMbtlnum( aes256.encrypt(usptCnvnSclpstHist.getChargerMbtlnum(), cnvnSclpstHistId));
		}
		if(CoreUtils.string.isNotBlank(usptCnvnSclpstHist.getChargerEmail())) {
			usptCnvnSclpstHist.setChargerEmail( aes256.encrypt(usptCnvnSclpstHist.getChargerEmail(), cnvnSclpstHistId));
		}
	...
	public void modifyCnvnApplcntHistFront(UsptCnvnApplcntHistDto usptCnvnApplcntHistDto,	List<MultipartFile> fileList) {
		...
		if(CoreUtils.string.isNotBlank(usptCnvnApplcntHist.getChargerMbtlnum())) {
			usptCnvnApplcntHist.setChargerMbtlnum(aes256.encrypt(usptCnvnApplcntHist.getChargerMbtlnum(), cnvnApplcntHistId));		/** 암호화된 담당자 휴대폰번호 */
		}
		if(CoreUtils.string.isNotBlank(usptCnvnApplcntHist.getChargerEmail())) {
			usptCnvnApplcntHist.setChargerEmail(aes256.encrypt(usptCnvnApplcntHist.getChargerEmail(), cnvnApplcntHistId));				/** 암호화된 담당자 이메일 */
		}
	...
	public UsptTaskRspnberHistDto selectTaskRspnberHistFront(FrontCnvnChangeParam frontCnvnChangeParam){
		...
		usptTaskRspnberHistBefore.setEncBrthdy(usptTaskPartcptsBefore.getEncBrthdy());				/** 복호화된 생년월일 */
	...
	public String modifyCnvnChangeReq(UsptCnvnChangeReq usptCnvnChangeReq, List<CmmtAttachment> attachFileDeleteListInput, List<MultipartFile> fileList) {
		BnMember worker = SecurityUtils.checkWorkerIsMember();
		...
		if(usptBsnsCnvnHistFileGrpupId !=null) {
		 ...
		 UsptBsnsCnvnHist  usptBsnsCnvnHist = new UsptBsnsCnvnHist();
		/* 첨부파일 삭제 */
		List<CmmtAttachment> attachFileDeleteList = attachFileDeleteListInput;
