...
import aicluster.batch.common.dao.CmmtEntDao;
import aicluster.batch.common.dao.CmmtMemberDao;
import aicluster.batch.common.dao.CmmtMemberDayStatsDao;
import aicluster.batch.common.dao.CmmtMemberHistDao;
import aicluster.batch.common.dao.CmmtMemberMonthStatsDao;
import aicluster.batch.common.dto.MemberStatsItem;
import aicluster.batch.common.entity.CmmtMember;
import aicluster.batch.common.entity.CmmtMemberDayStats;
...
public class MemberService {
	...
	@Autowired
	private CmmtMemberDayStatsDao cmmtMemberDayStatsDao;
	@Autowired
	private CmmtMemberMonthStatsDao cmmtMemberMonthStatsDayDao;
	@Autowired
	...
	public int updateBlackMemberToNormal() {
		...
		// 불량 회원 복원 대상 조회
		List<CmmtMember> list = cmmtMemberDao.selectList_blackMemberToNormal();

		/*
		 * 불량회원 복원
		 */
		List<CmmtMemberHist> histList = new ArrayList<>();
		CmmtMemberHist hist = null;
		Date now = new Date();
		String workCn = "회원상태가 변경되었습니다.(불량회원 > 정상)";
		for (CmmtMember cmmtMember : list) {
			// 복원
	...
	public int createMemberDayStats(String ymd) {
		...
		for (MemberStatsItem item : list) {
			CmmtMemberDayStats mds = findMemberDayStats(all, ymd, item.getMemberType());
			if (mds == null) {
				mds = new CmmtMemberDayStats();
				mds.setStatsYmd(ymd);
				mds.setMemberType(item.getMemberType());
				mds.setCreatedDt(now);
				all.add(mds);
	...
	private CmmtMemberDayStats findMemberDayStats(List<CmmtMemberDayStats> all, String ymd, String memberType) {
		for (CmmtMemberDayStats item : all) {
	...
	public int sendEmailToDormantMember() {
		...
		List<CmmtMember> list30 = cmmtMemberDao.selectList_lastLoginDt(day30);
		...
		List<CmmtMember> list7 = cmmtMemberDao.selectList_lastLoginDt(day7);
	 ...
	 * @return	발송건수
	 */
	private int sendEmail(List<CmmtMember> receiverList, String title, String emailCn) {
		if (receiverList == null || receiverList.size() == 0) {
			return 0;
		}
		List<EmailReceiver> list = new ArrayList<>();
		Map<String, String> templateParameter = null;
		for (CmmtMember cmmtMember : receiverList) {
	...
	public int deleteSecessionMember()
	{
		...
		List<CmmtMember> secesMemberList = cmmtMemberDao.selectList_memberSt(Code.memberSt.탈퇴, 7);

		// CMMT_MEMBER과 CMMT_ENT에서 삭제
		List<CmmtMemberHist> histList = new ArrayList<>();
		for (CmmtMember member : secesMemberList) {
			...
			// 이력정보 생성
			CmmtMemberHist hist = CmmtMemberHist.builder()
										.histId(string.getNewId(CodeExt.prefixId.이력ID))
										.histDt(now)
										.workTypeNm("회원탈퇴")
