...
public class MemberStBatchTask {
    ...
    public BatchReturn uptMemberSt() {
    	...
    	String fmt = "불량회원 복권: %d, 회원현황일별통계: %d, 회원현황월별통계: %d, 휴면회원전환대상자 이메일발송: %d, 휴면회원처리: %d, 탈퇴회원처리: %d";
    	String result = String.format(fmt, blackCnt, dayStatsCnt, monthStatsCnt, dormantEmailCnt, dormantCnt, secessionCnt);
