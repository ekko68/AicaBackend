...
import aicluster.common.common.dao.CmmtTermsDao;
...
import aicluster.framework.common.entity.CmmtTermsConsent;
...
public class TermsService {
	...
	@Autowired
	private CmmtTermsDao cmmtTermsDao;
	 ...
	 * @return 시행일 목록
	 */
	public JsonList<CmmtTermsListItem> getDayList(String termsType, String srchCd)
	{
		SecurityUtils.checkWorkerIsInsider();

		// 약관유형 시행일 목록 조회
		List<CmmtTermsListItem> list = cmmtTermsDao.selectList_beginDay(termsType, srchCd);
	...
	public TermsDetailDto insert(TermsParam param)
	{
		...
		List<CmmtTerms> termsSet = cmmtTermsDao.selectSet(param.getTermsType(), param.getBeginDay());
		if(!termsSet.isEmpty()) {
			throw new InvalidationException(String.format(validateMessageExt.조회결과있음, "동일한 시행일의 약관"));
		}

		// 필수약관 데이터 insert
		CmmtTerms requiredTerms = CmmtTerms.builder()
		...
		if (string.isNotBlank(param.getOptionedTermsCn())) {
			CmmtTerms optionedTerms = CmmtTerms.builder()
											.termsType(param.getTermsType())
											.beginDay(param.getBeginDay())
											.required(false)
	...
	public TermsDetailDto get(String termsType, String beginDay)
	{
		...
		List<CmmtTerms> termsSet = cmmtTermsDao.selectSet(termsType, beginDay);
		if(termsSet.isEmpty()) {
	...
	public void remove(String termsType, String beginDay)
	{
		...
		// 시행일에 해당하는 필수, 선택 약관정보 조회
		List<CmmtTerms> termsList = cmmtTermsDao.selectSet(termsType, beginDay);
		if(termsList.isEmpty()) {
			throw new InvalidationException(String.format(validateMessageExt.조회결과없음, "시행일의 약관정보"));
		}

		// 약관동의 정보가 존재하는지 검증(약관동의 정보가 있는 경우 삭제 금지)
		for (CmmtTerms terms : termsList) {
			List<CmmtTermsConsent> consentList = termsUtils.selectList(terms.getTermsType(), terms.getBeginDay(), terms.isRequired());
	 ...
	 */
	public JsonList<CmmtTerms> getTodayTerms(String termsType)
	{
		// 오늘 유효한 필수,선택 약관정보 조회
		List<CmmtTerms> list = cmmtTermsDao.select_today(termsType);
