...
public class EventService {
	...
	public CorePagination<CmmtEvent> getList(EventGetListParam param, CorePaginationParam pageParam)
	{
		...
		// 페이지 목록 조회
		List<CmmtEvent> list = cmmtEventDao.selectList(param, info.getBeginRowNum(), info.getItemsPerPage(), totalItems);
	...
	public CmmtEvent addEvent(CmmtEvent event, List<CmmtEventCn> eventCnList, MultipartFile pcThumbnailFile,
		...
		if (pcThumbnailFile != null && pcThumbnailFile.getSize() > 0) {
			CmmtAttachment pcThumbAtt = attachmentService.uploadFile_noGroup(config.getDir().getUpload(), pcThumbnailFile, CodeExt.uploadExt.imageExt, 0);
			pcThumbnailFileId = pcThumbAtt.getAttachmentId();
		}
		if (mobileThumbnailFile != null && mobileThumbnailFile.getSize() > 0) {
			CmmtAttachment mobileThumbAtt = attachmentService.uploadFile_noGroup(config.getDir().getUpload(), mobileThumbnailFile, CodeExt.uploadExt.imageExt, 0);
		...
		if (pcImageFile != null && pcImageFile.getSize() > 0) {
			CmmtAttachment pcImageAtt = attachmentService.uploadFile_noGroup(config.getDir().getUpload(), pcImageFile, CodeExt.uploadExt.imageExt, 0);
			pcImageFileId = pcImageAtt.getAttachmentId();
		}
		if (mobileImageFile != null && mobileImageFile.getSize() > 0) {
			CmmtAttachment mobileImageAtt = attachmentService.uploadFile_noGroup(config.getDir().getUpload(), mobileImageFile, CodeExt.uploadExt.imageExt, 0);
		...
		if (attachment != null && attachment.size() > 0) {
			CmmtAttachmentGroup attGruop = attachmentService.uploadFiles_toNewGroup(config.getDir().getUpload(), attachment, CodeExt.uploadExt.genericExt, 0);
