...
//@Slf4j
public class CmmtPopupDaoTest extends TestDaoSupport {
	@Autowired
	private CmmtPopupDao popupDao;

	private CmmtPopup popup;

	@BeforeEach
	public void init() {
		Date now = new Date();

		popup = CmmtPopup.builder()
				.popupId("popup-test")
	...
	public void test() {
		...
		// select
		CmmtPopup p1 = popupDao.select(popup.getPopupId());
		Assert.assertNotNull(p1);

		// selectList_time
		Date now = new Date();
		List<CmmtPopup> list = popupDao.selectList_time(CodeExt.popupType.메인화면, now);
		...
		list = popupDao.selectList(param, info.getBeginRowNum(), info.getItemsPerPage(), totalItems);
		Assert.assertTrue(list.size() > 0);
