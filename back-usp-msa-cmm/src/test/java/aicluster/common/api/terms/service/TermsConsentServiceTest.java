...
public class TermsConsentServiceTest extends TestServiceSupport {
	...
	private List<CmmtTerms> termsList;
	...
	void init() {
		...
		for (String termsType : TERMS_TYPES) {
			JsonList<CmmtTerms> cmmtTermsList = termsService.getTodayTerms(termsType);
	...
	void test() {
		...
		List<CmmtTermsConsent> consentList = new ArrayList<>();
		for (String termsType : TERMS_TYPES) {
			String beginDay = termsBeginDay.get(termsType);
			List<CmmtTermsConsent> termsConsentSet = termsUtils.selectList(termsType, beginDay, null, worker.getMemberId());
