...
public class EventViewService {
	 ...
	 * @return 이벤트 목록
	 */
	public JsonList<CmmtEvent> getTodayList(Long searchCnt) {
		if(searchCnt == null || searchCnt < 1) {
			searchCnt = 4L;
		}
		List<CmmtEvent> list = cmmtEventDao.selectList_time(searchCnt);
	 ...
	 * @return 페이징 목록
	 */
	public CorePagination<CmmtEvent> getList(EventViewGetListParam param, CorePaginationParam pageParam)
	{
		// 검색 조건 검증
		...
		List<CmmtEvent> list = cmmtEventDao.selectList_posting(param, info.getBeginRowNum(), info.getItemsPerPage(), totalItems);

		// CorePagination 생성하여 출력
		CorePagination<CmmtEvent> vo = new CorePagination<>(info, list);
		return vo;
	}
