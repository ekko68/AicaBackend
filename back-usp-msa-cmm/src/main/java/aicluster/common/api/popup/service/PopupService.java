...
import aicluster.common.common.dao.CmmtPopupDao;
...
public class PopupService {
	...
	@Autowired
	private CmmtPopupDao cmmtPopupDao;
	 ...
	 * @return 팝업정보
	 */
	public CmmtPopup select(String popupId)
	{
		CmmtPopup popup = cmmtPopupDao.select(popupId);
		if (popup == null) {
	...
	public CorePagination<CmmtPopup> getlist(PopupGetListParam param, CorePaginationParam pageParam)
	{
		...
		// 페이지 목록 조회
		List<CmmtPopup> list = cmmtPopupDao.selectList(param, info.getBeginRowNum(), info.getItemsPerPage(), totalItems);

		return new CorePagination<>(info, list);
	}

	public CmmtPopup addPopup(CmmtPopup popup, MultipartFile attachment) {
		...
		if (attachment != null && attachment.getSize() > 0) {
			...
			CmmtAttachment att = attachmentService.uploadFile_noGroup(config.getDir().getUpload(), attachment, exts, 0);
			attachmentId = att.getAttachmentId();
		}

		// 입력값 세팅
		CmmtPopup cmmtPopup = CmmtPopup.builder()
	...
	public CmmtPopup getPopup(String popupId) {
		...
		CmmtPopup popup = select(popupId);
	...
	public CmmtPopup modifyPopup(CmmtPopup popup, MultipartFile attachment) {
		...
		CmmtPopup cmmtPopup = cmmtPopupDao.select(popup.getPopupId());
		if (cmmtPopup == null) {
			throw new InvalidationException(String.format(validateMessageExt.조회결과없음, "팝업정보"));
		}
	...
	public void removePopup(String popupId) {
		...
		CmmtPopup cmmtPopup = cmmtPopupDao.select(popupId);
		if (cmmtPopup == null) {
			throw new InvalidationException(String.format(validateMessageExt.조회결과없음, "팝업정보"));
		}
	...
	public JsonList<CmmtPopup> getTodayList(String systemId) {
		...
		List<CmmtPopup> list = cmmtPopupDao.selectList_time(systemId, now);

		return new JsonList<>(list);
	}

	public CmmtPopup modifyStatus(String popupId, Boolean posting) {
