...
import aicluster.common.common.dao.CmmtBoardArticleCnDao;
import aicluster.common.common.dao.CmmtBoardArticleDao;
import aicluster.common.common.dao.CmmtBoardArticleUrlDao;
...
public class BoardArticleService {
	...
	@Autowired
	private CmmtBoardArticleCnDao cmmtBoardArticleCnDao;

	@Autowired
	private CmmtBoardArticleUrlDao cmmtBoardArticleUrlDao;
	...
	public CorePagination<BoardArticleListItem> getList(GetArticlesParam param, CorePaginationParam pageParam) {
		CmmtBoard cmmtBoard = cmmtBoardDao.select(param.getBoardId());
		if (cmmtBoard == null) {
		...
		// 고정고지 추가 (페이지 번호(page)가 첫 페이지(1)인 경우에만 출력한다.)
		if (cmmtBoard.getNoticeAvailable() == true && pageParam.getPage() == 1L) {
			List<BoardArticleListItem> noticeList = cmmtBoardArticleDao.selectList_notice(param.getBoardId(), param.getCategoryCd(), param.getPosting());
	...
	public CmmtBoardArticle add(
		...
		if (cmmtBoard.getCategory()) {
			if (string.isBlank(param.getCategoryCd())) {
				...
				CmmtCode code = cmmtCodeDao.select(codeGroup, param.getCategoryCd());
				if (code == null) {
		...
		if (cmmtBoard.getUseThumbnail()) {
			if (pcThumbnailFile != null && pcThumbnailFile.getSize() > 0) {
				...
				CmmtAttachment att = attachmentService.uploadFile_noGroup(config.getDir().getUpload(), pcThumbnailFile, exts, 0);
			...
			if (mobileThumbnailFile != null && mobileThumbnailFile.getSize() > 0) {
				String[] exts = CodeExt.uploadExt.imageExt;
				CmmtAttachment att = attachmentService.uploadFile_noGroup(config.getDir().getUpload(), mobileThumbnailFile, exts, 0);
		...
		if (cmmtBoard.getAttachable()) {
			if (attachments != null && attachments.size() > 0) {
				...
				CmmtAttachmentGroup attGroup = attachmentService.uploadFiles_toNewGroup(config.getDir().getUpload(), attachments, exts, size);
		...
		if (images != null && images.size() > 0) {
			String[] exts = CodeExt.uploadExt.imageExt;
			CmmtAttachmentGroup attGroup = attachmentService.uploadFiles_toNewGroup(config.getDir().getUpload(), images, exts, 0);
		...
		StringBuilder articleCn = new StringBuilder(param.getArticle());
		if (cmmtBoard.getUseForm()) {
			for (BoardArticleCnParam bap : param.getArticleCnList()) {
		...
		CmmtBoardArticle cmmtBoardArticle = CmmtBoardArticle.builder()
				.articleId(articleId)
		...
		if (param.getArticleCnList() != null && param.getArticleCnList().size() > 0) {
			List<CmmtBoardArticleCn> articleCnList = new ArrayList<>();
			long idx = 0;
			for (BoardArticleCnParam cn : param.getArticleCnList()) {
				CmmtBoardArticleCn cmmtBoardArticleCn = CmmtBoardArticleCn.builder()
		...
		if (param.getArticleUrlList() != null && param.getArticleUrlList().size() > 0) {
			List<CmmtBoardArticleUrl> articleUrlList = new ArrayList<>();
			long idx = 0;
			for (BoardArticleUrlParam up : param.getArticleUrlList()) {
				CmmtBoardArticleUrl bau = CmmtBoardArticleUrl.builder()
		...
		if (cmmtBoard.getUseForm()) {
			List<CmmtBoardArticleCn> articleCnList = cmmtBoardArticleCnDao.selectList(articleId);
			List<CmmtBoardArticleUrl> articleUrlList = cmmtBoardArticleUrlDao.selectList(articleId);
	 ...
	 * @return
	 */
	public CmmtBoardArticle get(String boardId, String articleId) {
		BnMember worker = SecurityUtils.getCurrentMember();
	 ...
	 * @return
	 */
	public CmmtBoardArticle modify(
		...
		// 게시글 조회
		CmmtBoardArticle cmmtBoardArticle = cmmtBoardArticleDao.select(param.getArticleId());
		if (cmmtBoardArticle == null) {
			throw new InvalidationException("게시글이 존재하지 않습니다.");
		}
		...
		cmmtBoardArticle.setArticle(param.getArticle());
		...
		if (cmmtBoard.getUseForm()) {
			List<CmmtBoardArticleCn> articleCnList = cmmtBoardArticleCnDao.selectList(param.getArticleId());
			List<CmmtBoardArticleUrl> articleUrlList = cmmtBoardArticleUrlDao.selectList(param.getArticleId());
			cmmtBoardArticle.setArticleCnList(articleCnList);
			cmmtBoardArticle.setArticleUrlList(articleUrlList);
		}
		cmmtBoardArticle.setCmmtBoard(cmmtBoard);
	 ...
	 * @return
	 */
	public CmmtBoardArticle select(String boardId, String articleId)
	{
		// 게시판 조회
		CmmtBoard cmmtBoard = cmmtBoardDao.select(boardId);
		if (cmmtBoard == null) {
