...
public class FrontCnvnCnclsService {
	...
	public FrontCnvnCnclsDto getBsnsCnvnInfo(FrontCnvnCnclsParam frontCnvnCnclsParam){
		...
		if (string.isNotBlank(usptBsnsCnvnResult.getCnvnFileGroupId())) {
			List<CmmtAttachment> list = attachmentService.getFileInfos_group(usptBsnsCnvnResult.getCnvnFileGroupId());
	 ...
	 * @return
	 */
	public void modifyPrtcmpnySign( FrontCnvnCnclsInputParam frontCnvnCnclsInputParam,  List<MultipartFile> fileList) {
		...
		/* 협약파일 삭제 */
		List<CmmtAttachment> attachFileDeleteList = frontCnvnCnclsInputParam.getAttachFileDeleteList();
		if( attachFileDeleteList !=null && attachFileDeleteList.size() >0) {
			for(CmmtAttachment attachInfo : attachFileDeleteList) {
				...
				if(!groupFileExists) {
					usptBsnsCnvn.setCnvnFileGroupId(null);
				}
			}
		}
