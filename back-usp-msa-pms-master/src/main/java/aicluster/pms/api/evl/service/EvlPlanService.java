...
//	private UsptExclMfcmmDao usptExclMfcmmDao;
	...
	public String insert(UsptEvlPlan usptEvlPlan) {
		...
		//평가계획 등록
		if(usptEvlPlanDao.selectListCount(usptEvlPlan) > 0L){
...
//				removeCmitTargetInfo(sectId);//해당 위원 관련 대상 테이블 모두 삭제
	...
	//평가단계 평가자료 목록
	public List<UsptEvlCmitDta> getEvlCmitDtaList(String evlPlanId) {
		return usptEvlCmitDao.selectCmitDtaList(evlPlanId);
	}


	//평가계획 평가자료 저장
	public UsptEvlCmitDta modifyEvlCmitDta(String evlPlanId, String evlCmitId, List<MultipartFile> attachments) {
		Date now = new Date();
		BnMember worker = SecurityUtils.checkWorkerIsInsider();
		...
		//첨부파일
		//평가위원회 첨부파일이 이미 있었는지 확인하기 attachmentGroupId 조회하기
		String attachmentGroupId = usptEvlCmitDao.selectCmitDtaInfo(evlCmitId);
		...
		usptEvlCmitDta.setEvlPlanId(evlPlanId);//평가계획ID
		usptEvlCmitDta.setEvlCmitId(evlCmitId);//평가위원회ID
	...
	public List<UsptEvlTrget> modifyEvlTargetBySectList(String evlPlanId, List<UsptEvlTrget> usptEvlTrgetList) {
		...
		for(UsptEvlTrget usptEvlTrgetListInfo : usptEvlTrgetList) {
			...
			if (string.equals(_flag, "I") || string.equals(_flag, "D") ) {
				...
				if (string.equals(_flag, "I") ) {
					...
					if(usptEvlTrgetDao.selectExistChkByPlanId(evlPlanId).size() > 0) {
		...
		for(UsptEvlTrget usptEvlTrgetListInfo : usptEvlTrgetList) {
			...
			if (string.equals(_flag, "I")) {
				...
				for(UsptEvlStep usptEvlStepInfo : usptEvlStepDao.selectList(evlPlanId, "1")) {
