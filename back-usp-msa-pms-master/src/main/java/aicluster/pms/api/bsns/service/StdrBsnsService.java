...
public class StdrBsnsService {
	...
	public JsonList<JobStepDto> getJobStepList(String stdrBsnsCd){
		SecurityUtils.checkWorkerIsInsider();
		List<JobStepDto> list = usptStdrJobStepDao.selectViewList(stdrBsnsCd);
		if(!list.isEmpty())
			list.stream().findFirst().get().setIsChecked(true);
		...
		for(UsptStdrAtchmnflSetup regInfo : setupList) {
			...
			if(regInfo.getFormatFileOrder() != -1) {
				if(fileList != null) {
					...
					CmmtAttachment att = attachmentService.uploadFile_noGroup(config.getDir().getUpload(), fileInfo, exts, 0);
