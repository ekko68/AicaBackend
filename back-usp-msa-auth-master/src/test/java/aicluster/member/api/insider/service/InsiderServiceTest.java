...
//	}
	...
	public void test() {
		...
		List<CmmtInsider> insInsiderRst = new ArrayList<>();
		for (InsiderMemberDto insider : initTempInsiderList) {
			CmmtInsider insInsider = service.addInsider(insider);
			insInsiderRst.add(insInsider);
		}

		for (CmmtInsider insider : insInsiderRst) {
			System.out.println("내부자\t\t" + insider.toString());
		}

		// 내부자 정보 조회
		System.out.println("***** 정보 조회 결과 *****");
		CmmtInsider selInsider = service.getInsider(insInsiderRst.get(0).getMemberId());
		...
		CmmtInsider uptInsider = service.modifyInsider(updateInsider);
