...
public class MemberServiceTest extends TestServiceSupport {
	...
	public void test() {
		...
		CmmtMember getMember = service.get(memberId);
		...
		CmmtMember modifyMember = service.modify(mp);
		...
		CmmtMember black = service.black(blackListDto);
		System.out.println("불량회원 등록 결과 :" + black.toString());
		System.out.println("\n\n\n");

		System.out.println("불량회원 해제");
		CmmtMember unblack = service.unblack(memberId);
...
//		System.out.println("회원탈퇴 결과 : " + secession.toString());
		...
		JsonList<CmmtMemberHist> memberHistList = service.getHistList(memberId);
