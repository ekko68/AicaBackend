...
//@Slf4j
public class CmmtAuthorityDaoTest extends TestDaoSupport {
	@Autowired
	private CmmtAuthorityDao dao;

	private CmmtAuthority cmmtAuthority;
	...
	public void test() {
		// 현재 건수
		List<CmmtAuthority> list = dao.selectAll();
		int size = list.size();
		...
		CmmtAuthority a2 = dao.select(cmmtAuthority.getAuthorityId());
