...
import aicluster.member.common.dao.CmmtPasswdHistDao;
...
public class SelfService {
	...
	@Autowired
	private CmmtPasswdHistDao cmmtPasswdHistDao;
	...
	public SessionKeyDto emailCertReq(String passwdCheckKey, String email)
	{
		...
		param.setTitle("(AICA) 인증번호");
		Map<String, String> templParam = new HashMap<>();
		templParam.put("certNo", certNo);
		param.addRecipient(email, worker.getMemberNm(), templParam);

		EmailResult er = emailUtils.send(param);
		if (er.getSuccessCnt() == 0) {
			throw new InvalidationException("이메일 인증번호 발송에 실패하였습니다. 잠시 후에 다시 이용해 주십시오.");
		}
	...
	public void modifyPasswd(ModifyPasswdParam param) {
		...
		// 비밀번호 조건 검사
		// 새 비밀번호 적합성 검사
		List<CmmtPasswdHist> passwdHistList = cmmtPasswdHistDao.selectList_recent(cmmtMember.getMemberId(), 3);
		 ...
		 * 비밀번호 이력 추가
		 */
		CmmtPasswdHist pwdHist = CmmtPasswdHist.builder()
	...
	public void modifyMobileCert(NiceIdResultParam param)
	{
         ...
         */
        CmmtMember member = cmmtMemberDao.selectByCi(result.getConnInfo());
        if (member == null) {
