...
public class AccountService {
    ...
    private CmmtMember getMember(SessionAccountCertDto certDto)
    {
        // 인증 CI 값으로 회원정보를 조회한다.
        CmmtMember memberForCi = memberDao.selectByCi(certDto.getCi());
        if (memberForCi == null) {
        	throw new InvalidationException("인증 정보로 가입된 회원정보가 없습니다.");
        }

        // 회원ID로 회원정보를 조회한다.
        CmmtMember member = memberDao.select(certDto.getMemberId());
        if (member == null) {
	...
	public void undomant(AccountParam param)
	{
		...
		// 회원정보를 가져온다.
        CmmtMember member = getMember(certDto);
	...
	public void unlock(AccountUnlockParam param)
	{
		...
		// 비밀번호 규칙 확인
		List<CmmtPasswdHist> passwdHistList = cmmtPasswdHistDao.selectList_recent(member.getMemberId(), 3);
