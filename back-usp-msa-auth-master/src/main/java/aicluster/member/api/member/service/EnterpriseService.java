...
import aicluster.member.common.dao.CmmtEntDao;
...
public class EnterpriseService {
	...
	public CmmtEnt add(EnterpriseParam enterParam) {
		Date now = new Date();
		...
		CmmtEnt cmmtEnt = cmmtEntDao.select(worker.getMemberId());
		if(cmmtEnt != null) {
		...
		CmmtMember cmmtMember = cmmtMemberDao.select(enterParam.getMemberId());
		if (cmmtMember == null) {
		...
		CmmtEnt selectEnt = cmmtEntDao.select(worker.getMemberId());
		return selectEnt;
	}
	...
	public CmmtEnt modify(EnterpriseParam enterParam) {
		...
		CmmtEnt selectEnt = cmmtEntDao.select(enterParam.getMemberId());
		return selectEnt;
	}
	...
	public CmmtEnt getMemberEnt(String memberId) {
		// 로그인 사용자가 내부사용자인지 검증한다.
		SecurityUtils.checkWorkerIsInsider();

		// memberId로 기업정보가 존재하는지 검사한다.
		CmmtEnt cmmtEnt = cmmtEntDao.select(memberId);
		if(cmmtEnt == null) {
			throw new InvalidationException(String.format(validateMessageExt.조회결과없음, "기업정보"));
		}
