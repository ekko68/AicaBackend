...
public class JoinService {
    ...
    @Autowired
    private CmmtMemberHistDao memberHistDao;
    ...
    public SessionKeyDto emailCertReq(String email) {
        ...
        param.setTitle("이메일 인증");
        param.setEmailCn(String.format("이메일 인증번호입니다.\n[%s]", certNo));
        param.addRecipient(email, email);
	...
	public SessionKeyDto phoneCertReq(String sessionId, String mobileNo)
	{
		...
		if (string.isBlank(smsCn)) {
			throw new InvalidationException("SMS내용 파일을 읽을 수 없습니다.");
		}
    ...
    public VerifyLoginIdResultDto verifyLoginId(String loginId)
    {
        ...
        // 로그인ID 건수 조회
        long duplicateCnt = memberDao.selectCount(loginId, null, null, null);
    ...
    private CertResultForJoinDto prcsFirstCert(NiceIdResultParam param)
    {
        ...
        CmmtMember member = memberDao.selectByCi(result.getConnInfo());
        if (member != null) {
            isDuplicate = true;
...
//		sessionUtils.updateExpiredDt(joinDto.getMobileCertSessionId(), 60);
    ...
    public CertResultForJoinDto verifyPkiCert(PkiCertParam param)
    {
        ...
        String sessionId = null;
        CmmtMember member = memberDao.selectByCi(result.getIdentifyData());
        if (member != null) {
            isDuplicate = true;
            loginId = member.getLoginId();
    ...
    public MemberSelfDto join(JoinParam param)
    {
        ...
        // 마케팅수신여부 값 세팅(개인정보수집동의서 - 선택을 동의한 경우에는 true, 그외에는 false로 한다)
		...
		member.setMarketingReception(개인정보수집_선택_동의정보.isConsentYn());
	...
	public void unsecession(String sessionId)
	{
        ...
        // 회원정보 조회
        Date now = new Date();
        CmmtMember member = memberDao.selectByCi(ci);
        if (member == null) {
