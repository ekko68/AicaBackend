...
import aicluster.member.common.dao.CmmtProgramDao;
import aicluster.member.common.dao.CmmtProgramRoleDao;
...
public class ProgramService {
	...
	@Autowired
	private CmmtProgramDao cmmtProgramDao;

	@Autowired
	private CmmtProgramRoleDao cmmtProgramRoleDao;
	...
	public JsonList<CmmtProgram> getList(String systemId, String programNm, String urlPattern) {
		...
		List<CmmtProgram> list = cmmtProgramDao.selectList(systemId, programNm, urlPattern);

		return new JsonList<>(list);
	}

	public CmmtProgram addProgram(ProgramDto dto) {
		...
		// programNm 중복 조회
		CmmtProgram cmmtProgram = cmmtProgramDao.selectByName(dto.getSystemId(), dto.getProgramNm());
		if (cmmtProgram != null) {
		...
		// 리스트 생성
		List<CmmtProgramRole> list = new ArrayList<>();
		for (String role : dto.getRoles()) {
			CmmtProgramRole cmmtProgramRole;
			cmmtProgramRole = CmmtProgramRole.builder()
					.programId(programId)
					.roleId(role)
		...
		CmmtProgram program = cmmtProgramDao.select(programId);

		// 정보 조회
		return program;
	}

	public CmmtProgram getProgram(String programId) {
		...
		CmmtProgram cmmtProgram = cmmtProgramDao.select(programId);
		if (cmmtProgram == null) {
	...
	public CmmtProgram modifyProgram(ProgramDto dto) {
		...
		// 중복 확인
		CmmtProgram cmmtProgram = cmmtProgramDao.select(dto.getProgramId());
		if (cmmtProgram == null) {
			throw new InvalidationException("자료가 없습니다.");
		}
		...
		CmmtProgram program = cmmtProgramDao.select(dto.getProgramId());
	...
	public void initData() throws UnsupportedEncodingException, ParseException, IOException
	{
		...
		for (ProgramDto program : programList)
		{
			...
			CmmtProgram cmmtProgram = addProgram(program);
			if (cmmtProgram != null) {
				insCnt++;
			}
		}
