...
import aicluster.member.common.dao.CmmtAuthorityRoleDao;
...
public class AuthorityService {
	...
	@Autowired
	private CmmtAuthorityRoleDao cmmtAuthorityRoleDao;
	@Autowired
	...
	public JsonList<CmmtAuthority> getList() {
		SecurityUtils.checkWorkerIsInsider();
		List<CmmtAuthority> cmmtAuthorityList = cmmtAuthorityDao.selectAll();
...
//		}
	...
	public CmmtAuthority add(AuthorityRequestDto param) {
		...
		cmmtAuthority = CmmtAuthority.builder()
		...
		if(roleId != null) {
			List<CmmtAuthorityRole> list = new ArrayList<>();
			for (int i = 0; i < roleId.length; i++) {
				CmmtAuthorityRole cmmtAuthorityRole;
				cmmtAuthorityRole =CmmtAuthorityRole.builder()
						.authorityId(authorityId)
						.roleId(roleId[i])
						.build();
	...
	public CmmtAuthority get(String authorityId) {
		SecurityUtils.checkWorkerIsInsider();
	...
	public CmmtAuthority modify(AuthorityRequestDto param) {
		String authorityId = param.getAuthorityId();
		Date now = new Date();
		BnMember worker = SecurityUtils.checkWorkerIsInsider();
		InvalidationsException inputValidateErrs = new InvalidationsException();
