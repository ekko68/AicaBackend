...
public class MemberHistUtils {
	 ...
	 * @param newMember
	 */
	public void addChangeHist(String workerId, CmmtMember oldMember, CmmtMember newMember) {
		List<CmmtMemberHist> list = new ArrayList<>();
		String workCn = null;
		CmmtMemberHist cmmtMemberHist = null;
		Date now = new Date();
		...
		if (!string.equals(oldMember.getMemberType(), newMember.getMemberType())) {
			...
			cmmtMemberHist = CmmtMemberHist.builder()
					.histId(string.getNewId("hist-"))
					.histDt(now)
					.workTypeNm("회원구분변경")
		...
		if (!string.equals(oldMember.getAuthorityId(), newMember.getAuthorityId())) {
			...
			CmmtAuthority authority1 = cmmtAuthorityDao.select(oldMember.getAuthorityId());
			CmmtAuthority authority2 = cmmtAuthorityDao.select(newMember.getAuthorityId());
			if (authority1 != null) {
	...
	public void addPasswdInitHist(String workerId, String memberId) {
		...
		CmmtMemberHist cmmtMemberHist = CmmtMemberHist.builder()
				.histId(string.getNewId("hist-"))
				.histDt(now)
				.workTypeNm("비밀번호 초기화")
				.memberId(memberId)
